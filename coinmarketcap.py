...
for data in result:
    if data['price_usd'] and data['24h_volume_usd'] and data['last_updated']:
        sql = """INSERT INTO coinmarket(coinid,symbol,price_usd,volume,last_updated,update_time)
              values ('%s','%s',%s,%s,%s,UNIX_TIMESTAMP() )""" % (data['id'],data['symbol'],data['price_usd'],data['24h_volume_usd'],data['last_updated'])
        print(sql)
        sql1 = """insert into coinmarket_last(coinid,symbol,price_usd,volume) values ('%s','%s',%s,%s)""" % (data['id'],data['symbol'],data['price_usd'],data['24h_volume_usd'])
        try:
            cursor.execute(sql)
            db.commit()
            cursor.execute(sql1)
