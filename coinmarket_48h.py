...
sql1 = """select symbol,price_usd,volume from coinmarket where update_time<=UNIX_TIMESTAMP()-48*3600 and update_time> UNIX_TIMESTAMP() -48*3600-600"""
cursor.execute(sql1)
...
for row in result:
    ...
    if symbol in coin:
        ...
        try:
            sql = """insert into coinmarket_48h(symbol,price,volume) values ('%s',%s,%s)""" % (
                symbol, price, volume)
            print(sql)
            cursor.execute(sql)
            db.commit()
